...
- `SchemaOptions` defaults `maxRegexPatternBytes` to 256,
  ...
  `maxValidationDepth` to 128, `maxRefResolutions` to 1024,
  ...
  ceiling. Validation depth has an absolute hard ceiling of 128, so larger
  configured values are clamped to 128. `trustedRegex()` removes only the regex
