...
`pJsonParser` parses **RFC 8259 JSON** into `pjson` values. This chapter shows
the resource limits that keep hostile documents from exhausting memory or the
call stack, the independent duplicate-key policy, and structured diagnostics. Follow along with
[`examples/src/05_parsing_and_errors.cpp`](../examples/src/05_parsing_and_errors.cpp).

## Parse options

Configure a reusable parser with `pJsonParser::Options`:

```cpp
struct pJsonParser::Options {
...
opt.maxNodes = 100000;
pJsonParser parser(opt);
pJsonParser::Error err;
pjson doc = parser.parse(text, err);
```
...
`pJsonParser::Options::duplicateKeys`. This changes only duplicate handling; it never
...
shallow.maxDepth = 3;
pJsonParser::Error err;
pjson d = pJsonParser(shallow).parse("[[[[1]]]]", err);   // fails: too deep
```

## Getting the error details

Pass a `pJsonParser::Error` to learn what went wrong. Reporting APIs reset every
...
```cpp
pJsonParser::Error err;
pjson doc = pJsonParser().parse("[1, 2, ]", err);
if (!err.ok) {
...
The parser retains its options, so reuse `parser.parse(text, err)` for multiple
inputs. Because a failed parse returns a
...
DOM-allocation failures produce a null `pjson` value plus optional `pJsonParser::Error`; SAX
...
- `pJsonParser::Error{ ok, offset, line, column, message }` tells you where and why
