...
//
//===----------------------------------------------------------------------===//
// PJSON-API-005 and PJSON-PARSE-002: the structured pJsonParser::Error::Code categories
...
namespace {
    ...
    struct AcceptingSaxHandler : pJsonParser::SaxHandler {};

    pJsonParser::Error::Code codeOf(const std::string& doc,
                                    const pJsonParser::Options& opt = pJsonParser::Options()) {
...
//===----------------------------------------------------------------------===//
// Each failure class maps to its stable pJsonParser::Error::Code.
//===----------------------------------------------------------------------===//
TEST(error_codes_classify_failure_categories) {
    CHECK_EQ(codeOf("[1,2,]"), pJsonParser::Error::Syntax);
    CHECK_EQ(codeOf("\"\\uD800\""), pJsonParser::Error::InvalidEncoding); // lone surrogate
    CHECK_EQ(codeOf("{\"a\":1,\"a\":2}"), pJsonParser::Error::DuplicateKey);
    CHECK_EQ(codeOf("18446744073709551616"), pJsonParser::Error::NumberRange); // > UINT64_MAX

    pJsonParser::Options depth;
    depth.maxDepth = 2;
    CHECK_EQ(codeOf("[[[1]]]", depth), pJsonParser::Error::DepthLimit);

    pJsonParser::Options input;
    input.maxInputBytes = 3;
    CHECK_EQ(codeOf("[1, 2, 3]", input), pJsonParser::Error::InputLimit);

    pJsonParser::Options nodes;
    nodes.maxNodes = 1;
    CHECK_EQ(codeOf("[1, 2, 3]", nodes), pJsonParser::Error::NodeLimit);
}
...
TEST(error_code_success_state) {
    ...
    CHECK(p != nullptr);
    CHECK(err.ok);
    CHECK_EQ(err.code, pJsonParser::Error::None);
...
TEST(error_code_null_input_is_invalid_argument) {
    ...
    CHECK(!err.ok);
    CHECK_EQ(err.code, pJsonParser::Error::InvalidArgument);

    AcceptingSaxHandler handler;
    CHECK(!pJsonParser().parseSax(static_cast<const char*>(nullptr), 5, handler, err));
    CHECK(!err.ok);
    CHECK_EQ(err.code, pJsonParser::Error::InvalidArgument);
}
...
TEST(duplicate_key_reported_early_at_key_offset) {
    ...
    CHECK(p == nullptr);
    CHECK(!err.ok);
    CHECK_EQ(err.code, pJsonParser::Error::DuplicateKey);
